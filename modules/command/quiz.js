...
module.exports.handleReaction = ({ api, event, handleReaction }) => {
	...
	if (response == handleReaction.answer) api.sendMessage("ধন্যবাদ আপনার Answer টি সঠিক হয়েছে,🥰", event.threadID);
	else api.sendMessage("Sorry আপনার Answer টি ভুল,🥱  :'<", event.threadID);
...
module.exports.run = async ({  api, event, args }) => {
	...
	return api.sendMessage(`Here is the question for you:\n        ${decodeURIComponent(fetch.data.results[0].question)}\n\n   👍: True       😢: False`, event.threadID, async (err, info) => {
		...
		if (data.answerYet !== 1) {
			api.sendMessage(`Time out!! সঠিক  answer টি হলো ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
