...
monitor.on('up', (res) => logger(`${res.address}:${res.port} IS ONLINE!!`, "[ JR-JAHID ]"));
...
app.listen(port, () =>
	logger(`Your app is listening a http://localhost:${port}`, "[ JR-JAHID ]")
     );      


logger("Opened server site...", "[ JR-JAHID ]");
logger("Opening Uptime Monitor by       JR-Jahid...", "[ Starting ]")
...
function startBot(message) {
    (message) ? logger(message, "[ JR-Jahid ]") : "";
  ...
  child.on("error", function(error) {
    logger("An error occurred: " + JSON.stringify(error), "[ JR-JAHID ]");
  });
...
axios.get("https://raw.githubusercontent.com/MR-RAHAD-511/Rahad-bypass2/main/package.json").then((res) => {
  logger(res['data']['name'], "[ JR-Jahid ]");
  logger("Version: " + res['data']['version'], "[ JR-Jahid ]");
  logger(res['data']['description'], "[ JR-Jahid ]");
});
startBot();
